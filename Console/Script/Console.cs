...
namespace ConsoleUtility {
    public class Console : MonoBehaviour {
        [SerializeField] ConsoleInput consoleInput;

        [Header("Items")] public Canvas Canvas;
        ...
        void OnEnable() {
            ...
            LogText.font.RequestCharactersInTexture(
                "qwertyuiopasdfghjklzxcvbnmQWERYTUIOPASDFGHJKLZXCVBNM1234567890~`!@#$%^&*()_+{}[]:;\"'/.,?><");
        ...
        public void UpdateAutoPanel() {
            ...
            if (s_ConsoleData.aliases.ContainsKey(words[0].ToUpper())) {
                ...
                sb.AppendFormat(
                    "<color=lime><b>Alias</b></color> <color=orange>{0}</color> => <color=yellow>{1}</color>:\n",
                    words[0], s_ConsoleData.aliases[words[0].ToUpper()]);
                var command =
                    s_ConsoleData.commands[
                        s_ConsoleData.aliases[words[0].ToUpper()].Split(' ').FirstOrDefault().ToUpper()];
                ...
                StringBuilder sb = new StringBuilder();
                foreach (var word in s_ConsoleData.aliases.Keys.Concat(s_ConsoleData.commands.Keys)
                             .Where(o => o.Contains(words[0].ToUpper())).ToArray()) {
                    bool isAlias = s_ConsoleData.aliases.ContainsKey(word);
                    string append = isAlias
                        ? string.Format("(alias for '<color=yellow>{0}</color>')",
                            s_ConsoleData.aliases[word.ToUpper()])
                        : s_ConsoleData.commands[word].summary;
        ...
        public static void Log(string Command, string Message, LogType type = LogType.Log, bool logToPeek = true) {
            ...
            foreach (string line in lines)
                s_ConsoleData.lines.Add("<color=gray>[" + Time.unscaledTime.ToString("F3") + "]</color> " + prepend +
                                        line);
            ...
            if (aliases != null)
                foreach (var alias in aliases) {
                    if (!alias.AliasString.Contains(" "))
                        ...
                        Debug.LogError(string.Format("Cannot add alias for command {0} : alias '{1}' contains spaces",
                            command.name, alias.AliasString));
                }
        }
            ...
            public void AutoRegisterConsoleCommands() {
                ...
                foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies()) {
                    foreach (Type type in assembly.GetTypes()) {
                        if (type.GetInterfaces().Contains(typeof(IConsoleCommand)) && type
                                .GetCustomAttributes(typeof(AutoRegisterConsoleCommandAttribute), true).Length > 0) {
...
Additional arguments are ignored";
    ...
    Usage: clear [console|views]
        ...
        public IEnumerable<Console.Alias> aliases {
            get { yield return Console.Alias.Get("cls", "clear"); }
        }
    }
